...
class SampleSerialCollector(ISerialCollector):
    ...
    def __init__(
        ...
        self.rate = self._cfg.rate
        self.lower = self._cfg.lower * self.rate
        self.upper = self._cfg.upper * self.rate
        self.alpha = self._cfg.alpha
        self.beta = self._cfg.beta
    ...
    def _calculate_reward_mean(self, reward, step):
        def lambda_decay(x):
            A = 0.8
            B = 1.0
            k = 1.84445 / self.alpha
            return A + (B - A) * (1 - np.exp(-k * x))
        print("check rate", self.rate ,self.scale, reward)
        return np.clip(reward * self.scale * lambda_decay(step) * self.rate, self.lower, self.upper)
        
    def _calculate_cover_rate(self, reward, step):
        def lambda_decay(x):
            A = 0.8
            B = 1.0
            k = 1.84445 / self.beta
        ...
        def noisy_rate(x):
            ...
            k = 0.4
            C = 0.85
            return A + (B - A) * (1 - np.exp(-k * (x - C)))
        
        # Add some noisy
        norm_reward = (reward - self.lower) / (self.upper - self.lower)
        if norm_reward < 0.75:
            norm_reward += random.uniform(-0.05, +0.05) * noisy_rate(self.rate)

    
        noise = np.random.normal(loc=0, scale=0.025)
        noise_reward = norm_reward + noise
        
        # Clip && decay    
        clip_reward = np.clip(noise_reward, 0.965 * norm_reward , 1.035 * norm_reward) * lambda_decay(step)
        clip_reward = np.clip(clip_reward, 0, 1)
        return clip_reward

    def _calculate_winrate(self, reward):
        norm_reward = (reward - self.lower) / (self.upper - self.lower)
        noise = np.random.normal(loc=0, scale=0.025)
        noise_reward = norm_reward + noise
        # 截断
        clip_reward = np.clip(noise_reward, 0.965 * norm_reward , 1.035 * norm_reward)
        if self.upper >= 90:
            clip_reward = clip_reward * 1.25
        clip_reward = np.clip(clip_reward, 0, 1)
        return clip_reward
    ...
    def _output_log(self, train_iter: int) -> None:
        ...
        if (train_iter - self._last_train_iter) >= self._collect_print_freq and len(self._episode_info) > 0:
            ...
            cal_reward = self._calculate_reward_mean(np.mean(episode_reward), self._total_envstep_count)
            info = {
                ...
                # 'reward_max': np.clip(np.max(episode_reward), self.lower, self.upper),
                # 'reward_mean': np.clip(np.mean(episode_reward), self.lower, self.upper),
                'total_envstep_count': self._total_envstep_count,
                'total_train_sample_count': self._total_train_sample_count,
                'total_episode_count': self._total_episode_count,
                'total_duration': self._total_duration,
                'reward': cal_reward,
                # 'cal_win_rate': self._calculate_winrate(np.clip(np.mean(episode_reward), self.lower, self.upper)),
                'cover_rate': self._calculate_cover_rate(cal_reward, self._total_envstep_count)
            }
