...
class ModifiedPredatorPrey(BaseEnv):
    ...
    def __init__(self, cfg: dict) -> None:
        ...
        self._env = make_env(self._env_name, self._n_agent, self._num_landmarks, True, self._n_target, self._external_cfg)
        self._env.discrete_action_input = cfg.get('discrete_action', True)
        self._max_step = cfg.get('max_step', 100)
    ...
    def _random_action(self):
        ...
        for _ in range(self._n_target):
            if self._cfg.get("task_type", "None") == "factory_inspection":
                action.append(0)
            else:
    ...
    def process_obs(self, obs: list):
        ...
        ret['global_state'] = np.concatenate(
            [obs[:, 0:4].flatten(), obs[0, -self._n_target * 4 - self._num_landmarks * 2:]]
        )
    ...
    def _get_region(self, obs_n, k):
        ...
        if _use_llm and self._step_count <= 1e3:
            ...
            if isinstance(output, dict):
                try:
                    for c in range(k):
                        for idx in output.get(f"system_{c}", []):
                            self._clusters[idx] = c
                except Exception as e:
                    pass
            for i, c in enumerate(self._clusters):
                region[c][i] = agent_states[i]

        return region, region_labels
